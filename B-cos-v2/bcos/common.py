...
__all__ = [
    ...
    "explanation_mode",
    "gradient_to_image",
    "gradient_to_image_shared_scale",
    "gradient_to_images_shared_scale",
    ...
    def explain(
        ...
        **grad2img_kwargs,
        ...
        create_graph : bool, optional
            If True, keep autograd graph for returned dynamic linear weights,
            allowing second-order gradients through W(x).
        grad2img_kwargs : Any
        ...
        with torch.enable_grad(), self.explanation_mode():
            # fwd + prediction
            out = self(in_tensor)  # (1, num_classes) = (1, 1000)
            pred_out = out.max(1) # (1,)
            result["prediction"] = pred_out.indices.item()

            # select output (logit) to explain
            if idx is None:  # explain prediction
                to_be_explained_logit = pred_out.values # take the predice class
        ...
        result["dynamic_linear_weights"] = dynamic_linear_weights
        result["contribution_map"] = (in_tensor * dynamic_linear_weights).sum(1)

        # generate (color) explanation
        result["explanation"] = gradient_to_image(
            in_tensor[0], dynamic_linear_weights[0], **grad2img_kwargs
        )

        return result, out # rewrite to return the logit
...
def gradient_to_image(image, linear_mapping, smooth=15, alpha_percentile=99.5): # linear mapping: mean Gradient
    """
    ...
    contribs = (image * linear_mapping).sum(0, keepdim=True)  # [1, H, W]
    ...
    # clip off values below1 0 (i.e., set negatively weighted channels to 0 weighting)
...
def _compute_bcos_rgb_and_alpha(
    image: Tensor,
    linear_mapping: Tensor,
    smooth: int,
) -> Tuple[Tensor, Tensor]:
    """
    Internal helper that computes RGB explanation channels and raw alpha map
    ...
    return rgb_grad, alpha


def gradient_to_image_shared_scale(
    image: Tensor,
    linear_mapping: Tensor,
    *,
    smooth: int = 15,
    alpha_denominator: Optional[Union[float, Tensor]] = None,
    alpha_percentile: float = 99.5,
) -> np.ndarray:
    """
    Same visualization as `gradient_to_image`, but allows external alpha scaling.

    This is useful when you want multiple explanations to share one normalization
    scale so they are directly comparable in brightness/transparency.
    ...
    linear_mapping: Tensor
        Dynamic linear mapping. Shape [C, H, W].
    smooth: int
        Kernel size for alpha smoothing.
    alpha_denominator: Optional[float | Tensor]
        Shared normalization denominator for alpha. If None, computed from this
        single sample using `alpha_percentile`.
    alpha_percentile: float
        Used only when `alpha_denominator` is None.

    Returns
    -------
    np.ndarray
        RGBA explanation image with shape [H, W, 4].
    """
    rgb_grad, alpha_raw = _compute_bcos_rgb_and_alpha(image, linear_mapping, smooth=smooth)

    if alpha_denominator is None:
        alpha_denominator_t = torch.quantile(alpha_raw, q=alpha_percentile / 100)
    else:
        alpha_denominator_t = torch.as_tensor(alpha_denominator, device=alpha_raw.device, dtype=alpha_raw.dtype)

    alpha = (alpha_raw / (alpha_denominator_t + 1e-12)).clip(0, 1)
    rgba = torch.cat([rgb_grad, alpha], dim=0).permute(1, 2, 0)
    return rgba.detach().cpu().numpy()


def gradient_to_images_shared_scale(
    images: Tensor,
    linear_mappings: Tensor,
    *,
    smooth: int = 15,
    alpha_percentile: float = 99.5,
) -> np.ndarray:
    """
    Compute multiple RGBA explanations using one shared alpha normalization scale.
    ...
    linear_mappings: Tensor
        Batch of dynamic linear mappings. Shape [N, C, H, W].
    smooth: int
        Kernel size for alpha smoothing.
    alpha_percentile: float
        Percentile used to compute one shared alpha denominator across all N maps.

    Returns
    -------
    np.ndarray
        Batch of RGBA explanation images with shape [N, H, W, 4].
    """
    if images.ndim != 4 or linear_mappings.ndim != 4:
        raise ValueError("Expected images and linear_mappings to have shape [N, C, H, W].")
    if images.shape != linear_mappings.shape:
        raise ValueError("images and linear_mappings must have the same shape.")
    if images.shape[0] < 1:
        raise ValueError("Expected batch size N >= 1.")

    rgb_grads: List[Tensor] = []
    alphas_raw: List[Tensor] = []
    for img_i, lin_i in zip(images, linear_mappings):
        rgb_i, alpha_i = _compute_bcos_rgb_and_alpha(img_i, lin_i, smooth=smooth)
        rgb_grads.append(rgb_i)
        alphas_raw.append(alpha_i)

    alpha_stack = torch.stack(alphas_raw, dim=0)  # [N, 1, H, W]
    shared_denominator = torch.quantile(alpha_stack, q=alpha_percentile / 100)

    rgba_list: List[np.ndarray] = []
    for rgb_i, alpha_i in zip(rgb_grads, alphas_raw):
        alpha_norm = (alpha_i / (shared_denominator + 1e-12)).clip(0, 1)
        rgba_i = torch.cat([rgb_i, alpha_norm], dim=0).permute(1, 2, 0)
        rgba_list.append(rgba_i.detach().cpu().numpy())

    return np.stack(rgba_list, axis=0)
