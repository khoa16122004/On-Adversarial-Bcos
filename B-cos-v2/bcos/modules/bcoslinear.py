...
class BcosLinear(DetachableModule):
    ...
    def forward(self, in_tensor: Tensor) -> Tensor:
        ...
        """
        # Simple linear layer
        out = self.linear(in_tensor)  # \hat{w} ^ T . x
        ...
        norm = LA.vector_norm(in_tensor, dim=-1, keepdim=True) + 1e-12 # ||x||

        # Calculate the dynamic scale (|cos|^(B-1))
        # Note that cos = \hat{w} ^ T . x / ||x|| - out / norm
        ...
        out = dynamic_scaling * out  # |cos|^(B-1) (ŵ·x) = |cos|^(B-1)  
        return out
